...
async function clearEmptyTables() {
  console.log('🗑️  清空空表...\n')
  ...
  for (const tableName of emptyTables) {
    try {
      ...
      if (error) {
        console.log(`   ❌ 清空表 ${tableName} 失败: ${error.message}`)
      } else {
        console.log(`   ✅ 表 ${tableName} 已清空`)
      }
      
    } catch (err) {
      console.log(`   ⚠️  清空表 ${tableName} 时出错: ${err.message}`)
    }
  }
...
// 由于Supabase的限制，我们需要直接在SQL编辑器中执行DDL语句
function outputSQLStatements() {
  console.log('📄 请在Supabase Dashboard -> SQL Editor 中执行以下SQL语句:\n')
...
`
  ...
  console.log('3. 复制上述SQL语句粘贴到编辑器')
  console.log('4. 点击 "RUN" 执行')
  console.log('5. 确认所有表创建成功\n')
}

async function main() {
  await clearEmptyTables()
  outputSQLStatements()
}
