...
test('手动Excel上传测试', async ({ page }) => {
  ...
  // 上传文件
  const testFile = path.join(__dirname, '数据', 'test file.xlsx');
  ...
  console.log('页面是否包含文件名:', pageContent.includes('test file.xlsx'));
  ...
  // 查找文件名显示
  const fileNameElements = await page.locator('text=/test file\\.xlsx/').count();
