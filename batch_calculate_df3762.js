/**
 * DF-3762员工批量计算脚本 - 验证C类→B类→A类三阶段员工分类逻辑
 * 计算2023年1-12月和2024年1-9月，共21个月的数据
 * DF-3762: 入职2023年3月6日，跨越三个员工类别的特殊案例
 */
...
async function getEmployeeAverageSalary(employeeId, year, assumption) {
  ...
  console.log(`📊 ${employeeId} ${year}年平均工资计算:`)
  console.log(`  记录数量: ${yearRecords.length}个月`)
  console.log(`  总工资: ¥${totalSalary.toFixed(2)}`)
  console.log(`  平均工资: ¥${(totalSalary / yearRecords.length).toFixed(2)}`)
...
async function getEmployeeFirstMonthSalary(employeeId, assumption) {
  ...
  console.log(`📊 ${employeeId} 入职首月工资:`)
  console.log(`  首月: ${firstRecord.salary_month}`)
  console.log(`  ${salaryField}: ¥${firstRecord[salaryField]}`)
  
  return firstRecord[salaryField] || 0
}

/**
 * 选择参考工资和确定类别 (修正版) - 重点验证C类员工逻辑
 */
async function selectReferenceWageAndCategory(employeeId, category, calculationMonthStr, assumption) {
  const calculationYear = parseInt(calculationMonthStr.substring(0, 4))
  const calculationMonthNum = parseInt(calculationMonthStr.substring(4, 6))
  
  console.log(`🔍 参考工资选择: ${employeeId} ${category}类 ${calculationYear}年${calculationMonthNum}月`)
  
  if (category === 'A') {
    // A类员工（老员工）- 根据4个期间分别处理
    if (calculationYear === 2023 && calculationMonthNum <= 6) {
      // 2023年1-6月：特殊替代规则，使用2022年月平均工资
      const wage = await getEmployeeAverageSalary(employeeId, 2022, assumption)
      return {
        wage,
        category: '2022年平均工资'
      }
      ...
      const wage = await getEmployeeAverageSalary(employeeId, 2023, assumption)
      return {
        wage,
        category: '2023年平均工资'
      }
    ...
    const wage = await getEmployeeFirstMonthSalary(employeeId, assumption)
    return {
      wage,
      category: '入职首月工资'
    }
...
function calculateInsuranceAdjustedBases(referenceWageBase, rules) {
  ...
  const injury = {
    original_base: referenceWageBase,
    floor: rules.injury_base_floor || 0,
    cap: rules.injury_base_cap || 999999,
...
async function calculateSingleMonth(employeeId, calculationMonthStr, assumption) {
  try {
    ...
    console.log(`👤 员工类别: ${category}类 (2023年3月入职，验证C类→B类→A类逻辑)`)
...
/**
 * 批量计算DF-3762员工所有月份 - 验证C类员工三阶段分类算法
 */
async function batchCalculateDF3762() {
  const employeeId = 'DF-3762'
  const assumption = 'wide' // 只计算宽口径
  
  console.log(`🚀 开始批量计算员工 ${employeeId} (${assumption}口径)`)
  console.log(`👤 DF-3762: 2023年3月入职，预期展示C类→B类→A类三阶段分类`)
  console.log(`📅 计算范围: 2023年1月-2024年9月 (共21个月)\n`)
  ...
  results.forEach(r => {
    const year = parseInt(r.calculation_month.substring(0, 4))
    const month = parseInt(r.calculation_month.substring(4, 6))
    if (year === 2023 && month <= 6) periodStats['2023_H1']++
    else if (year === 2023 && month >= 7) periodStats['2023_H2']++
    else if (year === 2024 && month <= 6) periodStats['2024_H1']++
    else if (year === 2024 && month >= 7) periodStats['2024_H2']++
  })
  ...
  const categoryStats = {}
  results.forEach(r => {
    const period = r.calculation_month.substring(0, 6)
    if (!categoryStats[period]) categoryStats[period] = new Set()
    categoryStats[period].add(r.employee_category)
  })
  
  console.log(`📊 员工分类三阶段验证:`)
  Object.entries(categoryStats).forEach(([period, categories]) => {
    console.log(`  ${period}: ${Array.from(categories).join(',')}类`)
  })
  
  // 分析参考工资类别分布
  const wageStats = {}
  results.forEach(r => {
    if (!wageStats[r.reference_wage_category]) wageStats[r.reference_wage_category] = 0
    wageStats[r.reference_wage_category]++
  })
  
  console.log(`📊 参考工资类别分布 (预期：C/B类均用入职首月，A类用平均工资):`)
  Object.entries(wageStats).forEach(([category, count]) => {
    console.log(`  ${category}: ${count}个月`)
  })
...
// 执行批量计算
batchCalculateDF3762().catch(error => {
