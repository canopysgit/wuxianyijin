/*
  Bulk insert 2024 H2 (202407-202409) narrow-calculation rows into
  calculate_result_2024_h2_narrow for employees who have salary
  records in 2024 H2. If reference base missing (avg_2023 & first
  both null), skip and write to a local JSON file for review.

  Usage:
    node scripts/insert_all_2024h2_narrow.js
*/
...
async function getBaselines(employeeIds) {
  ...
  for (let i = 0; i < employeeIds.length; i += chunkSize) {
    ...
    const { data, error } = await supabase
      ...
      .select('employee_id, hire_date, avg_wage_narrow_2023, first_month_base_narrow')
...
async function deleteExisting(employeeIds) {
  ...
  for (let i = 0; i < employeeIds.length; i += chunkSize) {
    ...
    const { error } = await supabase
      .from('calculate_result_2024_h2_narrow')
      .delete()
      .in('employee_id', chunk)
...
async function main() {
  ...
  console.log('🚀 Bulk insert 2024 H2 narrow for employees with H2 salary records (202407–202409)')
  ...
  for (const empId of empIds) {
    ...
    const avg23 = (b.avg_wage_narrow_2023 == null ? null : Number(b.avg_wage_narrow_2023))
    ...
    if (refBase == null || isNaN(refBase)) {
      missings.push({ period: '2024H2', assumption: 'narrow', employee_id: empId, hire_date: b.hire_date, preferred_source: pref, details: 'avg_wage_narrow_2023 and first_month_base_narrow both null' })
      continue
    }
  ...
  if (missings.length) {
    const file = path.join(process.cwd(), `missing_refs_2024H2_narrow_${Date.now()}.json`)
