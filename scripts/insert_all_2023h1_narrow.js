/*
  Bulk insert all employees' 2023 H1 (202301-202306) narrow-calculation rows
  into calculate_result_2023_h1_narrow, for employees who have salary
  records in 2023 H1 (salary_month_std between '202301' and '202306').

  Usage:
    node scripts/insert_all_2023h1_narrow.js
*/
...
function months2023H1() {
  return ['202301','202302','202303','202304','202305','202306']
}
...
async function getEmployeeIdsWithH1Salary() {
  const { data, error } = await supabase
    .from('salary_records')
    .select('employee_id')
    .gte('salary_month_std', '202301')
    .lte('salary_month_std', '202306')
  if (error) throw new Error('query salary_records failed: ' + error.message)
  const set = new Set()
  for (const r of data || []) set.add(r.employee_id)
...
async function deleteExisting(employeeIds) {
  ...
  for (let i = 0; i < employeeIds.length; i += chunkSize) {
    ...
    const { error } = await supabase
      .from('calculate_result_2023_h1_narrow')
      .delete()
      .in('employee_id', chunk)
...
async function main() {
  ...
  console.log('🚀 Bulk insert 2023 H1 narrow for employees with H1 salary records')
  ...
  const empIds = await getEmployeeIdsWithH1Salary()
  console.log(`👥 Employees in 2023H1 salary: ${empIds.length}`)
  if (!empIds.length) {
  ...
  for (const empId of empIds) {
    const b = baselines.get(empId)
    if (!b) continue
    const hireYear = new Date(b.hire_date).getUTCFullYear()
    const refBase = round2(hireYear < 2022 ? (Number(b.avg_wage_narrow_2022) || 0) : (Number(b.first_month_base_narrow) || 0))
    const refCat = hireYear < 2022 ? '2022年平均工资' : '入职首月工资'

    for (const ym of months2023H1()) {
  ...
  console.log('🧹 Deleting existing rows for all employees in range 202301-202306 ...')
  await deleteExisting(empIds)
  console.log(`📝 Inserting ${rows.length} rows ...`)
  await insertRows(rows)

  const sec = ((Date.now() - start) / 1000).toFixed(2)
  console.log(`✅ Done. Employees=${empIds.length}, Rows=${rows.length}, Time=${sec}s`)
}
