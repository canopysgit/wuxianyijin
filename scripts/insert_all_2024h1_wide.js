/*
  Bulk insert 2024 H1 (202401-202406) wide-calculation rows into
  calculate_result_2024_h1_wide for employees who have salary
  records in 2024 H1. If reference base missing (avg_2022 & first
  both null), skip and write to a local JSON file for review.

  Usage:
    node scripts/insert_all_2024h1_wide.js
*/
...
async function deleteExisting(employeeIds) {
  ...
  for (let i = 0; i < employeeIds.length; i += chunkSize) {
    ...
    const { error } = await supabase
      .from('calculate_result_2024_h1_wide')
      .delete()
      .in('employee_id', chunk)
...
async function main() {
  ...
  console.log('🚀 Bulk insert 2024 H1 wide for employees with H1 salary records')
  ...
  for (const empId of empIds) {
    ...
    if (refBase == null || isNaN(refBase)) {
      missings.push({ period: '2024H1', assumption: 'wide', employee_id: empId, hire_date: b.hire_date, preferred_source: pref, details: 'avg_wage_wide_2022 and first_month_base_wide both null' })
      continue
    }
  ...
  if (missings.length) {
    const file = path.join(process.cwd(), `missing_refs_2024H1_wide_${Date.now()}.json`)
