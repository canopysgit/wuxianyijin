/*
  Insert a single employee's 2023 H2 (202307-202312) wide-calculation rows
  into calculate_result_2023_h2_wide, using employee_baseline + policy_rules.

  Usage:
    node scripts/insert_employee_2023h2_wide.js DF-3615
    node scripts/insert_employee_2023h2_wide.js 3615   // will try DF-3615 automatically
*/
...
async function main() {
  ...
  if (!arg) { console.error('Usage: node scripts/insert_employee_2023h2_wide.js <EMPLOYEE_ID>'); process.exit(1) }

  console.log(`⚙️  (Re)insert 2023H2 wide for ${arg}`)
  ...
  // determine effective months: from first_insurance_month (or computed) within 2023H2
  ...
  const avg22 = (base.avg_wage_wide_2022 == null ? null : Number(base.avg_wage_wide_2022))
  const first = (base.first_month_base_wide == null ? null : Number(base.first_month_base_wide))
  ...
  if (refBase == null || isNaN(refBase)) throw new Error('Both avg_wage_wide_2022 and first_month_base_wide are null; cannot compute')
  ...
  for (const ym of months) {
    ...
    rows.push({
      ...
      injury_base_floor: injuryAdj, // not-null columns; mirror ref
