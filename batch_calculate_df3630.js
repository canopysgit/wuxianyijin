/**
 * DF-3630员工批量计算脚本 - 验证B类员工的计算逻辑
 * 计算2023年1-12月和2024年1-9月，共21个月的数据
 * DF-3630: 入职2022年8月5日，B类员工，有25个月记录
 */
...
async function calculateSingleMonth(employeeId, calculationMonthStr, assumption) {
  try {
    ...
    console.log(`👤 员工类别: ${category}类 (2022年入职，验证B类逻辑)`)
...
/**
 * 批量计算DF-3630员工所有月份 - 验证B类员工算法
 */
async function batchCalculateDF3630() {
  const employeeId = 'DF-3630'
  const assumption = 'wide' // 只计算宽口径
  
  console.log(`🚀 开始批量计算员工 ${employeeId} (${assumption}口径)`)
  console.log(`👤 DF-3630: 2022年8月入职，预期为B类员工`)
...
// 执行批量计算
batchCalculateDF3630().catch(error => {
