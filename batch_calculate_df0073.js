/**
 * DF-0073员工批量计算脚本 - 验证A类老员工的计算逻辑
 * 计算2023年1-12月和2024年1-9月，共21个月的数据
 * DF-0073: 入职2007年10月11日，A类老员工，有完整33个月记录
 */
 ...
 * 批量计算DF-0073员工所有月份
 */
async function batchCalculateDF0073() {
  const employeeId = 'DF-0073'
  ...
  // 验证参考工资切换逻辑
  console.log(`\n🔍 验证参考工资切换逻辑:`)
  const switchResults = results.filter(r => {
    const year = parseInt(r.calculation_month.substring(0, 4))
    const month = parseInt(r.calculation_month.substring(4, 6))
    return (year === 2024 && month === 6) || (year === 2024 && month === 7)
  })
  
  switchResults.forEach(r => {
    console.log(`  ${r.calculation_month}: ${r.reference_wage_category} = ¥${r.reference_wage_base.toFixed(2)}`)
  })
...
// 执行批量计算
batchCalculateDF0073().catch(error => {
